...
const AttachmentCard = ({ attachment, downloading, onDownload }: {
  ...
  <div className="mt-3 min-w-0 rounded-lg border border-border bg-background/80 p-3 shadow-card">
...
const MessageBubble = ({ message, downloadingId, onDownload }: {
  ...
  return (
    <div data-testid={`live-message-${message.id}`} className={`min-w-0 max-w-[92%] rounded-xl border p-3 sm:max-w-[82%] ${message.direction === "outbound" ? "ml-auto border-accent/30 bg-accent/10" : "mr-auto border-border bg-card"}`}>
      ...
      {message.text && <p className="mt-2 whitespace-pre-line break-words text-sm leading-6 text-foreground [overflow-wrap:anywhere]">{message.text}</p>}
      {attachments.map((attachment) => <AttachmentCard key={attachment.id} attachment={attachment} downloading={downloadingId === attachment.id} onDownload={onDownload} />)}
    </div>
  );
};
...
export const LiveConversationsModule = ({ email }: LiveConversationsModuleProps) => {
  ...
  useEffect(() => {
    ...
    const poll = window.setInterval(() => { if (document.visibilityState === "visible") void loadConversations(controller.signal); }, 10_000);
    return () => {
      controller.abort();
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      try {
        const response = await fetchLiveConversationMessages(email, selectedId, controller.signal);
  ...
  const filtered = useMemo(() => {
    ...
    return conversations.filter((conversation) => `${conversation.contactName} ${conversation.number ?? ""} ${conversation.chatId ?? ""} ${conversation.lastMessagePreview ?? ""}`.toLowerCase().includes(normalized));
  }, [conversations, query]);
  ...
  const toggleAi = async () => {
    ...
    try {
      ...
      setConversations((current) => current.map((item) => item.id === selected.id ? { ...item, aiEnabled: updated.aiEnabled } : item));
      setDetail((current) => current && current.conversation.id === selected.id
